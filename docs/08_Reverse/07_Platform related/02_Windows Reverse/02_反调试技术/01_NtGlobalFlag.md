...
1. 注册表`HKLM\System\CurrentControlSet\Control\SessionManager`的`GlobalFlag`的值会替换进行`NtGlobalFlag`字段. 尽管它随后还可能由Windows改变(以下会介绍), 注册表键值会对系统中所有进程产生影响并在重启后生效.
    ...
    ![GlobalFlag.png](../../../../_assets/reverse-platform-windows-anti-debug-figure-globalflag.png)
...
![Manually-set-peb-ntglobalflag.png](../../../../_assets/reverse-platform-windows-anti-debug-figure-manually_set_peb_ntglobalflag.png)
