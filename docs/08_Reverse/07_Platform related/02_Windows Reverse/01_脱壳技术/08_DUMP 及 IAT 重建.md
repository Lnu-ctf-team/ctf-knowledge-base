...
![right_click.jpg](../../../../_assets/reverse-platform-windows-unpack-figure-fix_iat-right_click.jpg)

弹出一个窗口, 看一下地址是否正确, 主要就是看看`入口点地址`有没有选对. 然后取消勾选`重建输入表`.

![dump.png](../../../../_assets/reverse-platform-windows-unpack-figure-fix_iat-dump.png)
...
![importrec.png](../../../../_assets/reverse-platform-windows-unpack-figure-fix_iat-importrec.png)
...
![auto_search.png](../../../../_assets/reverse-platform-windows-unpack-figure-fix_iat-auto_search.png)

我们点击`"Get Imports"`按钮便可以重建`IAT`. 左侧会显示`IAT`中各导入函数的地址以及是否有效. 显然在图中可以看到`ImportREC`找到了内存中`IAT`的位置并检测出各个函数都是有效的.

![get_imports.png](../../../../_assets/reverse-platform-windows-unpack-figure-fix_iat-get_imports.png)
