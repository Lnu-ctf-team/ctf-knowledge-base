...
这里需要联系上篇的 [手动查找IAT并使用ImportREC重建](./09_手动查找%20IAT%20并使用%20ImportREC%20重建.md)
...
![12.png](../../../../_assets/reverse-platform-windows-unpack-figure-unpack_dll-upx-dll-unpack-12.png)

我们将`UnpackMe.dll`后缀名改成`UnpackMe.exe`, 然后用OD载入.

![13.png](../../../../_assets/reverse-platform-windows-unpack-figure-unpack_dll-upx-dll-unpack-13.png)
...
`Dll`脱壳的基本步骤跟`exe`文件脱壳一样, 而在重建`IAT`时, 需要照着上篇 [手动查找IAT并使用ImportREC重建](./09_手动查找%20IAT%20并使用%20ImportREC%20重建.md) 所说的那样, 手动找到`IAT`表并用`ImportREC`进行重建. 只是要注意, 在脱壳完dump后, 要记得用LordPE把`DLL`标志恢复过来并将文件后缀名改为`.dll`.
