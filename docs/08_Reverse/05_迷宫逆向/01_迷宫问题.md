...
![setmaze.png](../../_assets/reverse-maze-figure-setmaze.png)

我们可以到`.rodata`段用光标选择所有的地图字符串, 按下`shift+E`提取所有的地图数据. 

![extract_maze.png](../../_assets/reverse-maze-figure-extract_maze.png)
...
![maze_origin.png](../../_assets/reverse-maze-figure-maze_origin.png)

再来看迷宫移动所需要的字符:

![choice.png](../../_assets/reverse-maze-figure-choice.png)

这里我们知道, 可以使用的字符有`L/R/U/D`, 分别对应于`左/右/上/下`.  

再往下看

![check.png](../../_assets/reverse-maze-figure-check.png)

通过调试是可以知道, 这里其实是每次在用户输入`L/R/U/D`后, 先打印一次你的输入, 然后打印对应的`X/Y坐标`. 而最后的判定成功的条件, 就是当`pos_x == 89 && pos_y == 28`. 那么我们就可以根据上述信息, 获得走出迷宫的路径. 

![maze.jpg](../../_assets/reverse-maze-figure-maze.jpg)
